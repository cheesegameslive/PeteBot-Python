...
def stackImage(topImg, botImg,w, resample=Image.BICUBIC, resize_big_image=True):
    botImg.convert("RGBA")
    _topImg = topImg
    _botImg = botImg
...
def stackGif(topImg,backImg,w):
    ...
    for frame in ImageSequence.Iterator(backImg):
        ...
        if w:
            ...
            dst.alpha_composite(topImg)
        frames.append(dst)
    frames[0].save('bubble.gif', save_all=True, append_images=frames[1:],loop=1)
